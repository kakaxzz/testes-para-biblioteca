...
export default function TesteIsbn() {
  ...
  async function buscarLivro(codigo?: string) {
    ...
    if (res.ok) {
      ...
      // Preenche os campos editáveis com os dados que vieram da API
      setCapaManual(data.capa || "")
      setAutorManual(data.autor || "")
      setSinopseManual(data.sinopse || "")
      // Transforma o array de assuntos em uma string separada por vírgula para editar
      setCategoriasManual(data.assuntos ? data.assuntos.join(", ") : "")
  ...
  async function salvarLivro() {
    ...
    // Monta o objeto final com tudo o que você editou nos inputs
    const livroFinal = {
      ...livro,
      autor: autorManual,
      sinopse: sinopseManual,
      capa: capaManual,
      // Transforma a string de volta em array para o banco de dados
      assuntos: categoriasManual.split(",").map(cat => cat.trim()).filter(cat => cat !== "")
    }
        ...
        <div style={{ marginTop: 20, borderTop: '1px solid #ccc', paddingTop: 20 }}>
          ...
          {capaManual && (
            <div style={{ marginBottom: 20 }}>
              <img src={capaManual} width={150} alt="Preview da Capa" style={{ borderRadius: 8 }} />
            </div>
          )}
