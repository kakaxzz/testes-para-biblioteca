...
export default function AdminLivros() {
  ...
  // "categorias" agora é string[] em vez de string
  const [campos, setCampos] = useState({
    capa: "", autor: "", sinopse: "", categorias: [] as string[],
    editora: "", edicao: "", cdd: "", cutter: "", volume: "",
  })
 
  // ── categorias do modal de edição ──
  // o modal usa editando.assuntos (string separada por vírgula, vinda do banco)
  // e editandoCategorias (string[]) para o componente
  ...
  // Sincroniza editandoCategorias quando abre o modal de edição
  useEffect(() => {
    if (editando) {
      const cats = editando.assuntos
        ? editando.assuntos.split(",").map((c: string) => c.trim()).filter(Boolean)
        : []
  ...
  async function buscarLivro(codigo?: string) {
    ...
    if (res.ok) {
      ...
      // Tenta mapear as categorias da API para as da lista pré-definida
  ...
  async function salvarLivro() {
    const res = await fetch("/api/livros", {
      ...
      body: JSON.stringify({
        ...
        capa: campos.capa,
        // Envia o array diretamente — a API já faz join(", ") internamente
  ...
  async function salvarEdicao() {
    const res = await fetch(`/api/livros/${editando.isbn}`, {
      ...
      body: JSON.stringify({
        ...editando,
        // Reconverte o array de volta para string antes de enviar
          ...
          <div className="card" style={{ maxWidth: 700 }}>
            ...
            {livro && (
              <div style={{ borderTop: "2px solid #f0eded", paddingTop: 24 }}>
                ...
                {/* ── CAMPO SUBSTITUÍDO: Combobox de categorias ── */}
            ...
            {editando && (
              <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", backdropFilter: "blur(4px)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <div className="modal-surface">
                  ...
                  {/* ── CAMPO SUBSTITUÍDO no modal de edição ── */}
                  <div className="input-group">
                    <label>Assuntos / Categorias</label>
                    <CategoriasSelect
